...
const ESTADOS: { value: EstadoGestion; label: string }[] = [
  { value: 'encuestado', label: 'Encuestado' }, { value: 'fin_gestion', label: 'Fin de gestión' },
  { value: 'no_acepta_encuesta', label: 'No acepta encuesta' }, { value: 'rellamar', label: 'Rellamar' },
  { value: 'pendiente', label: 'Pendiente' }, { value: 'no_es_titular', label: 'No es titular' },
]
...
export default function GestionModal({ cliente, perfil, onClose }: Props) {
  ...
  async function loadGestion() {
    ...
    if (data && data.length > 0) {
      ...
      setForm(prev => ({ ...prev, estado: g.estado ?? 'pendiente', fecha_rellamar: g.fecha_rellamar ?? '', motivo_rellamar: g.motivo_rellamar ?? '', observaciones: g.observaciones ?? '',
  ...
  function nextStep() { const e = validateStep(step); if (e.length) { setErrors(e); return }; setErrors([]); setStep(s => Math.min(s+1,3)) }
  function prevStep() { setErrors([]); setStep(s => Math.max(s-1,1)) }

  async function guardar() {
    const e = validateStep(3); if (e.length) { setErrors(e); return }
    setSaving(true)
    const supabase = createClient()
    const payload: any = {
    ...
    } catch(err) { console.error(err); setSaving(false); setErrors(['Error al guardar. Intentá de nuevo.']) }
  }
      ...
      <div style={{ background: '#fff', borderRadius: '14px', width: '100%', maxWidth: '660px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #E2E0D8', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: '12.5px', color: '#6B6A64', marginTop: '4px' }}>{cliente.marca} {cliente.modelo} · {cliente.concesionaria}{cliente.fecha_compra && ` · Compra: ${format(new Date(cliente.fecha_compra), 'dd/MM/yyyy')}`}</p>
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px', flexWrap: 'wrap' }}>
              {cliente.telefono && <a href={`tel:${cliente.telefono}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '12.5px', color: '#1B4F8A', textDecoration: 'none', background: '#DDE9F8', padding: '3px 10px', borderRadius: '20px' }}>📞 {cliente.telefono}</a>}
              {cliente.telefono_alternativo && <a href={`tel:${cliente.telefono_alternativo}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '12.5px', color: '#1B4F8A', textDecoration: 'none', background: '#DDE9F8', padding: '3px 10px', borderRadius: '20px' }}>📞 {cliente.telefono_alternativo}</a>}
              {cliente.email && <a href={`mailto:${cliente.email}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '12.5px', color: '#2D6A4F', textDecoration: 'none', background: '#D8F3DC', padding: '3px 10px', borderRadius: '20px' }}>✉️ {cliente.email}</a>}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            {historial.length > 0 && <button onClick={() => setShowHistorial(!showHistorial)} style={{ background: showHistorial ? '#1A1917' : 'none', border: '1px solid #E2E0D8', borderRadius: '6px', padding: '5px 12px', fontSize: '12px', cursor: 'pointer', color: showHistorial ? '#fff' : '#6B6A64', fontFamily: 'DM Sans' }}>📋 Historial ({historial.length})</button>}
        ...
        {showHistorial && (
          <div style={{ background: '#F0EFE9', borderBottom: '1px solid #E2E0D8', padding: '16px 24px', maxHeight: '180px', overflowY: 'auto' }}>
            <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.8px', color: '#9E9C95', marginBottom: '10px' }}>Historial</div>
            {historial.map(h => (
              <div key={h.id} style={{ display: 'flex', gap: '10px', marginBottom: '6px', fontSize: '12.5px' }}>
                <span style={{ color: '#9E9C95', fontFamily: 'DM Mono', flexShrink: 0 }}>{format(new Date(h.created_at), 'dd/MM HH:mm')}</span>
        ...
        {errors.length > 0 && <div style={{ background: '#FAE0E0', border: '1px solid #F09595', margin: '16px 24px 0', borderRadius: '6px', padding: '10px 14px', fontSize: '13px', color: '#8B2020' }}>{errors.map(e => <div key={e}>• {e}</div>)}</div>}
          ...
          <div>
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E0D8' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                ...
                ].map(item => <VerifyRow key={item.field} label={item.label} value={item.value} verified={(form as any)[item.field]} corregido={(form as any)[item.corrField]} corrType="text" onVerify={(v: boolean) => setField(item.field, v)} onCorrect={(v: string) => setField(item.corrField, v)} />)}
              </div>
            </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {ESTADOS.map(op => (
                  <button key={op.value} onClick={() => setField('estado', op.value)}
                    style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '13px', cursor: 'pointer', border: '1px solid', borderColor: form.estado === op.value ? '#1A1917' : '#E2E0D8', background: form.estado === op.value ? '#1A1917' : '#fff', color: form.estado === op.value ? '#F5F4F0' : '#6B6A64', fontFamily: 'DM Sans' }}>
                    {op.label}
                  </button>
                ))}
              </div>
        ...
        {step === 2 && (
          <div>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E0D8' }}>
              <SectionTitle>Atención del vendedor</SectionTitle>
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E0D8' }}>
              <SectionTitle>Atención administrativa</SectionTitle>
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E0D8' }}>
              <SectionTitle>Información sobre el vehículo</SectionTitle>
            ...
            <div style={{ padding: '20px 24px' }}>
              <SectionTitle>Postventa</SectionTitle>
        ...
        {step === 3 && (
          <div>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E0D8' }}>
              <SectionTitle>Contacto posterior</SectionTitle>
            ...
            <div style={{ padding: '20px 24px' }}>
              <SectionTitle>Observaciones libres</SectionTitle>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.8px', color: '#9E9C95', marginBottom: '14px' }}>{children}</div>
}
...
function VerifyRow({ label, value, verified, corregido, corrType, onVerify, onCorrect }: any) {
  return (
    <div style={{ marginBottom: '10px' }}>
      ...
      {verified === false && <input type={corrType} value={corregido} onChange={e => onCorrect(e.target.value)} placeholder={`${label} correcto...`} style={{ width: '100%', marginTop: '6px', background: '#FFFBF0', border: '1px solid #D08700', borderRadius: '6px', padding: '7px 11px', fontFamily: 'DM Mono', fontSize: '13px', outline: 'none', color: '#1A1917', boxSizing: 'border-box' }} />}
    </div>
  )
}
    ...
    <div style={{ marginBottom: '14px', opacity: disabled ? 0.5 : 1 }}>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        {options.map((op: string, i: number) => (
          <button key={op} onClick={() => !disabled && onChange(op)} style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '13px', cursor: disabled ? 'not-allowed' : 'pointer', border: '1px solid', borderColor: value === op ? '#1A1917' : '#E2E0D8', background: value === op ? '#1A1917' : '#fff', color: value === op ? '#fff' : '#6B6A64', fontFamily: 'DM Sans' }}>{labels[i]}</button>
        ))}
      </div>
    </div>
  )
