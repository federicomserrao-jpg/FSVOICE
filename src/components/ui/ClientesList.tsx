...
const FILTROS = [
  { key: 'todos', label: 'Todos' }, { key: 'pendiente', label: 'Pendiente' },
  { key: 'encuestado', label: 'Encuestado' }, { key: 'rellamar', label: 'Rellamar' },
  { key: 'no_acepta_encuesta', label: 'No acepta' }, { key: 'fin_gestion', label: 'Fin de gestión' },
...
export default function ClientesList({ clientes, gestiones, perfil, stats, filtroInicial, onRefresh }: Props) {
  ...
  const getUltimoEstado = (cliente: any): EstadoGestion => {
    ...
    return [...cliente.gestiones].sort((a: any, b: any) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())[0].estado
  }
  ...
  function exportarExcel() {
    const rows = gestiones.map(g => ({
      ...
      'Dirección original': g.cliente?.direccion ?? '', 'Dirección corregida': g.direccion_corregida ?? '',
      'Concesionaria': g.cliente?.concesionaria ?? '',
      'Marca original': g.cliente?.marca ?? '', 'Marca corregida': g.marca_corregida ?? '',
      'Modelo original': g.cliente?.modelo ?? '', 'Modelo corregido': g.modelo_corregido ?? '',
      'Patente original': g.cliente?.patente ?? '', 'Patente corregida': g.patente_corregida ?? '',
      'Estado': ESTADO_LABELS[g.estado as EstadoGestion] ?? g.estado, 'Operador': g.operador?.nombre ?? '',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtrados.length === 0 ? (
                ...
                const scoreAvg = [...(c.gestiones ?? [])].sort((a: any, b: any) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())[0]?.score_recomendacion
                return (
                  <tr key={c.id} style={{ borderBottom: '1px solid #E2E0D8' }}
                    ...
                    <td style={{ padding: '11px 14px', fontSize: '13px' }}>{scoreAvg ? `${scoreAvg}/5` : '—'}</td>
                    <td style={{ padding: '11px 14px' }}>
